        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                to="/contact"
                aria-label="Request a free digital media planning consultation"
                className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-blue-700 transform hover:scale-105 transition-all duration-300 shadow-lg"
              >
                Request Free Consultation
              </Link>
              <Link
                to="/contact"
                aria-label="Explore the full list of our digital media planning services"
                className="inline-flex items-center px-8 py-4 bg-white text-indigo-700 font-semibold rounded-xl hover:bg-gray-50 border border-indigo-200 transform hover:scale-105 transition-all duration-300 shadow-lg"
              >
                Explore Digital Media Services
              </Link>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service, index) => (
              <div
                key={index}
                className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
              >
          ...
          <div className="py-16">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {packages.map((pkg, index) => (
                <div
                  key={index}
                  className={`rounded-2xl p-8 relative ${pkg.popular
                    ? 'bg-gradient-to-br from-indigo-500 to-blue-600 text-white transform scale-105'
                    : 'bg-gradient-to-br from-gray-50 to-blue-50'} hover:shadow-xl transition-all duration-300`}
                >
                  {pkg.popular && (
                  ...
                  <div className="text-center mb-8">
                    ...
                    <div className="flex items-baseline justify-center">
                      ...
                      <span className={`ml-1 ${pkg.popular ? 'text-indigo-100' : 'text-gray-600'}`}>
                        {pkg.period}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-4 mb-8">
                    {pkg.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start">
                        <div
                          className={`w-2 h-2 rounded-full mt-2 mr-3 ${pkg.popular ? 'bg-indigo-200' : 'bg-indigo-500'}`}
                        />
                  ...
                  <Link
                    to="/contact"
                    aria-label={`Start with the ${pkg.name} digital media planning package`}
                    className={`block w-full text-center py-3 px-6 rounded-xl font-semibold transition-all duration-300 ${pkg.popular
